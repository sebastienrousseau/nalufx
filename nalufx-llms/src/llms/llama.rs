...
pub fn get_llama_api_key() -> Result<String, &'static str> {
    ...
    match dotenv() {
        Ok(_) => match env::var("LLAMA_API_KEY") {
            Ok(key) => Ok(key),
            Err(_) => {
...
/// Sends a POST request to the Llama API with the provided request body.
///
/// # Arguments
///
/// * `client` - A reference to the reqwest::Client instance used for making HTTP requests.
/// * `api_url` - A string representing the URL of the Llama API endpoint.
...
/// # Errors
///
/// * If the JSON response cannot be parsed into the `LlamaResponse` struct, an error is returned with an
