...
pub fn get_mistral_api_key() -> Result<String, &'static str> {
    ...
    match dotenv() {
        Ok(_) => match env::var("MISTRAL_API_KEY") {
            ...
            Err(_) => {
                ...
                Err("MISTRAL_API_KEY not found in the .env file")
            },
        },
        Err(err) => {
...
pub fn parse_mistral_response(body: &str) -> Result<Vec<f64>, Box<HttpResponse>> {
